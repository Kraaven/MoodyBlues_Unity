...
`BluesSessionManager`, `BluesRuntimeManager`, `Serializer`, `TransformEventDispatcher` in
`Assets/Scripts`) over a WebSocket connection. It is intended to be sufficient, on its own, for
an independent backend implementation to decode every byte the Unity client sends.
...
- Client connects to the backend: `ws://localhost:8765` (Unity is the client, backend is the
  server).
...
| WebSocket URL | `ws://localhost:8765` | `BluesStreamer.ConnectAsync` |
